...
A case you have already seen stays on this phone (`localStorage`). The draw prefers stories where the article states a taka amount. Most stories with no amount stay in the file and are not drawn. When the draw is finished, the screen says so in the language you chose. It does not start the list over.
